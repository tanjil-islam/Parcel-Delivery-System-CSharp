...
{
    public partial class Employee_CompleteOrder : Form
    {
        string employeename;
        ...
        public Employee_CompleteOrder(Panel mainpanel,string username)
        {
            InitializeComponent();
        ...
        private void btnCompleteDelivery_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0 && dataGridView1.SelectedRows[0].Cells["Cust_Name"].Value.ToString().Length != 0)
            {
                ...
                int rowsAffected = completeOrder.MarkAsComplete();

                if (rowsAffected > 0)
                {
                    MessageBox.Show("Number of orders Complete. Congrats " + custName);
                }
                else
                {
                    MessageBox.Show("Opps! Sorry " + custName);
                }
        ...
        public void showDetails()
        {
            try
            {
                ...
                SqlDataAdapter sda = new SqlDataAdapter("Select * from TransitOrdersInfo", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
